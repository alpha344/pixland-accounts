# Claude Code Configuration - PixLand Accounts Server

This file contains server-specific information and preferences for Claude Code development tasks.

## Purpose & Background

This repository serves as the centralized account management system for PixLand's microservice-based architecture. It handles **users, workspaces, roles, and permission evaluations** across all services.
...
- `make test` - Run unit tests
  - Use `PIXLAND_ACCOUNTS_DB=mongodb://localhost:27017` for custom MongoDB URL
...
`pixland-accounts` exposes **GraphQL APIs** to centrally manage key domain models:
...
> Note: These domain models are **owned and managed solely within `pixland-accounts`**. Other services must not manipulate them directly. Currently, some operations are in migration — full centralization is the intended goal.

### Example Usage in Other Services

For instance, the `dashboard` service contains a `getMe` use case that needs workspace information. Instead of maintaining local workspace logic, it sends a **GraphQL request to `pixland-accounts`** to fetch it.
...
All other services are expected to adopt the same model: rely on GraphQL APIs from `pixland-accounts` when interacting with user/workspace/role/permittable data.

## Permission Evaluation

### Overview & Flow

Each service performs **authorization via a GraphQL request** to the `pixland-accounts` service.
...
This way, the calling service **only needs to send the service/resource/action**, and `pixland-accounts` performs the actual evaluation.
...
- `pixland-accounts` loads the synced policy for evaluation
