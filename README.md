...
PixLand Accounts follows Domain-Driven Design (DDD) principles with clear layer separation:
...
This service is designed to run alongside other PixLand microservices within a shared Docker network. It provides centralized authentication and authorization for all PixLand services including pixland-visualizer, pixland-cms, pixland-flow, and others.

**Prerequisites:**

Before starting this service, ensure the following are running:
- The `pixland` Docker network
- MongoDB instance named `pixland-mongo` on that network

These are typically provided by any PixLand service (e.g., [pixland-visualizer](https://github.com/pixair-technologies/pixland-visualizer), [pixland-cms](https://github.com/pixair-technologies/pixland-cms), [pixland-flow](https://github.com/pixair-technologies/pixland-flow)).

Example setup with pixland-visualizer:

```bash
# Clone and start pixland-visualizer (or any other PixLand service)
git clone https://github.com/pixair-technologies/pixland-visualizer.git
cd pixland-visualizer/server
make run
```

**Start the service:**

Once the `pixland` network and `pixland-mongo` are available, start this service:
...
- Start PixLand Accounts server on port 8090
- Attach to the external `pixland` Docker network, making the service accessible to all PixLand microservices
- Connect to the `pixland-mongo` database

The GraphQL endpoint will be available at:
- From host machine: `http://localhost:8090/graphql`
- From within Docker network: `http://pixland-accounts-dev:8090/graphql`

**Note:** The service uses `docker-compose.dev.yml` which declares `networks.pixland.external: true`, meaning it attaches to the existing `pixland` network rather than creating its own.
...
```env
PIXLAND_ACCOUNTS_DB=mongodb://localhost:27017
PIXLAND_ACCOUNTS_DB_NAME=pixland-accounts
PIXLAND_AUTH0_DOMAIN=your-auth0-domain
PIXLAND_AUTH0_AUDIENCE=your-auth0-audience
...
PIXLAND_ACCOUNTS_DB=mongodb://localhost:27017 make test
